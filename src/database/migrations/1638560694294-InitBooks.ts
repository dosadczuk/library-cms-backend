...
export class InitBooks1638560694294 implements MigrationInterface {
  name = 'InitBooks1638560694294';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "copies" ("id" SERIAL NOT NULL, "number" character varying(50) NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "modified_at" TIMESTAMP DEFAULT now(), "removed_at" TIMESTAMP, "book_id" integer NOT NULL, CONSTRAINT "UQ_644b268e582671566ae8f78f5fe" UNIQUE ("number"), CONSTRAINT "PK_e75e3715bd8cd3329397ba0e4dc" PRIMARY KEY ("id")); COMMENT ON COLUMN "copies"."id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "copies"."number" IS 'Numer'; COMMENT ON COLUMN "copies"."created_at" IS 'Moment utworzenia rekordu'; COMMENT ON COLUMN "copies"."modified_at" IS 'Moment modyfikacji rekordu'; COMMENT ON COLUMN "copies"."removed_at" IS 'Moment usunięcie rekordu'; COMMENT ON COLUMN "copies"."book_id" IS 'Identyfikator rekordu'`,
    );
    await queryRunner.query(
      `CREATE TABLE "genres" ("id" SERIAL NOT NULL, "value" character varying(100) NOT NULL, CONSTRAINT "UQ_aaa21d013c0f7479debd7d21a0b" UNIQUE ("value"), CONSTRAINT "PK_80ecd718f0f00dde5d77a9be842" PRIMARY KEY ("id")); COMMENT ON COLUMN "genres"."id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "genres"."value" IS 'Wartość'`,
    );
    await queryRunner.query(
      `CREATE TABLE "languages" ("id" SERIAL NOT NULL, "value" character varying(100) NOT NULL, CONSTRAINT "UQ_212a72d3830f7841097c30bea48" UNIQUE ("value"), CONSTRAINT "PK_b517f827ca496b29f4d549c631d" PRIMARY KEY ("id")); COMMENT ON COLUMN "languages"."id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "languages"."value" IS 'Wartość'`,
    );
    await queryRunner.query(
      `CREATE TABLE "publishers" ("id" SERIAL NOT NULL, "name" character varying(250) NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "modified_at" TIMESTAMP DEFAULT now(), CONSTRAINT "UQ_39082806f986a63cd7dcf1782a5" UNIQUE ("name"), CONSTRAINT "PK_9d73f23749dca512efc3ccbea6a" PRIMARY KEY ("id")); COMMENT ON COLUMN "publishers"."id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "publishers"."name" IS 'Nazwa'; COMMENT ON COLUMN "publishers"."created_at" IS 'Moment utworzenia rekordu'; COMMENT ON COLUMN "publishers"."modified_at" IS 'Moment modyfikacji rekordu'`,
    );
    await queryRunner.query(
      `CREATE TABLE "ratings" ("id" SERIAL NOT NULL, "value" integer NOT NULL, "comment" text NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "modified_at" TIMESTAMP NOT NULL DEFAULT now(), "book_id" integer NOT NULL, CONSTRAINT "PK_0f31425b073219379545ad68ed9" PRIMARY KEY ("id")); COMMENT ON COLUMN "ratings"."id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "ratings"."value" IS 'Wartość'; COMMENT ON COLUMN "ratings"."comment" IS 'Komentarz'; COMMENT ON COLUMN "ratings"."created_at" IS 'Moment utworzenia rekordu'; COMMENT ON COLUMN "ratings"."modified_at" IS 'Moment modyfikacji rekordu'; COMMENT ON COLUMN "ratings"."book_id" IS 'Identyfikator rekordu'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "files" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" character varying(150) NOT NULL, "path" character varying(250) NOT NULL, "size" integer NOT NULL, "mime" character varying(50) NOT NULL, "checksum" character varying(50) NOT NULL, CONSTRAINT "PK_6c16b9093a142e0e7613b04a3d9" PRIMARY KEY ("id")); COMMENT ON COLUMN "files"."id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "files"."name" IS 'Nazwa'; COMMENT ON COLUMN "files"."path" IS 'Ścieżka na dysku'; COMMENT ON COLUMN "files"."size" IS 'Rozmiar'; COMMENT ON COLUMN "files"."mime" IS 'Typ MIME'; COMMENT ON COLUMN "files"."checksum" IS 'Suma kontrolna'`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."books_type_enum" AS ENUM('book', 'magazine', 'article', 'thesis')`,
    );
    await queryRunner.query(
      `CREATE TABLE "books" ("id" SERIAL NOT NULL, "isbn" character varying(13) NOT NULL, "type" "public"."books_type_enum" NOT NULL, "title" character varying(255) NOT NULL, "description" text, "issue_date" date NOT NULL, "pages" integer NOT NULL, "details" jsonb NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "modified_at" TIMESTAMP DEFAULT now(), "removed_at" TIMESTAMP, "publisher_id" integer NOT NULL, "genre_id" integer NOT NULL, "language_id" integer NOT NULL, "image_id" uuid, CONSTRAINT "UQ_54337dc30d9bb2c3fadebc69094" UNIQUE ("isbn"), CONSTRAINT "REL_ce3191ad6f325cb5b184d656dd" UNIQUE ("image_id"), CONSTRAINT "PK_f3f2f25a099d24e12545b70b022" PRIMARY KEY ("id")); COMMENT ON COLUMN "books"."id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "books"."isbn" IS 'ISBN'; COMMENT ON COLUMN "books"."type" IS 'Rodzaj'; COMMENT ON COLUMN "books"."title" IS 'Tytuł'; COMMENT ON COLUMN "books"."description" IS 'Opis'; COMMENT ON COLUMN "books"."issue_date" IS 'Data wydania'; COMMENT ON COLUMN "books"."pages" IS 'Liczba stron'; COMMENT ON COLUMN "books"."details" IS 'Szczegóły'; COMMENT ON COLUMN "books"."created_at" IS 'Moment utworzenia rekordu'; COMMENT ON COLUMN "books"."modified_at" IS 'Moment modyfikacji rekordu'; COMMENT ON COLUMN "books"."removed_at" IS 'Moment usunięcia rekordu'; COMMENT ON COLUMN "books"."publisher_id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "books"."genre_id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "books"."language_id" IS 'Identyfikator rekordu'; COMMENT ON COLUMN "books"."image_id" IS 'Identyfikator rekordu'`,
    );
    await queryRunner.query(
